...
def load_json_from_file(file_name, default_format):
    try:
        ...
        write_json_from_file(file_name, default_format)
        return default_format

def write_json_from_file(file_name, dictionary):
        ...
        while(message_not_sent):
            try:
                ...
                if "chat not found" in str(e):
                    ...
                    write_json_from_file("options.json", options_dict)
...
{immobiliare_last_room}
    ...
    def get_anticaptcha_page(driver, url):
        return get_page(driver, get_anticaptcha_url(url))
    ...
    def get_rooms_from_idealista():
        ...
        page = get_anticaptcha_page(driver, URLS['idealista'])
        ...
        write_json_from_file('status.json', status_dict)
    ...
    def get_rooms_from_subito():
        ...
        page = get_anticaptcha_page(driver, URLS['subito'] + "&gndr=1")
        ...
        page = get_anticaptcha_page(driver, URLS['subito'] + "&gndr=3")
        ...
        for room in rooms:
            ...
            room_name = room.find("h2", {"class": "BigCard-module_card-title__Cgcnt"}).get_text()
    ...
    def get_rooms_from_immobiliare():
        ...
        page = get_page(driver, URLS['immobiliare'])

        rooms = list(filter(lambda x: not x.select(".nd-figure"), page.select(".nd-mediaObject__content")))
        for room in rooms:
            ...
            room_price = room.find("li", {"class": "in-realEstateListCard__features--main"}).get_text()
            room_phone = "None"
        ...
        new_houses = dict_dif(rooms_data, load_json_from_file('status.json', STATUS_DEFAULT_FORMAT)['immobiliare']['last_check'])
        print(json.dumps(new_houses,indent = 4) + "\n")
    ...
    while(True):
        ...
        time.sleep((10 + random.randint(-3, 3)) * 60)
